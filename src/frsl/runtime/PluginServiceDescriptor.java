...
public class PluginServiceDescriptor implements IPluginServiceDescriptor {
	 ...
	 * 
	 * @param pluginServiceModel The Plugin Service Model
	 * @param parent             the Plugin Descriptor
	 */
	public PluginServiceDescriptor(PluginServiceModel pluginServiceModel, IPluginDescriptor parent) {
	...
	public IPluginService getServiceClass() {
		if (this.pluginService == null) {
			...
			IPluginClassLoader currentClassLoader = parent.getPluginClassLoader();

			Log.debug("[" + this.pluginServiceModel.getId() + "]" + ", " + "[" + currentClassLoader.toString() + "]");
			try {
				this.pluginService = (IPluginService) currentClassLoader.loadClass(className).getDeclaredConstructor()
						.newInstance();
